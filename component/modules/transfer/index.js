...
function appTransfer(config) {
  const appInst = Aotoo(Transfer, Actions)
  config.props.rendered = _rendered(appInst, config.rendered)
  appInst.setConfig(config)
  appInst.groupSelected = 'transfer_0'
  appInst.selected = []
  appInst.extend({
    // 选中项传送到另一边
    toggle(select){
      ...
      if (!select) {
        if (this.selected.length) {
          select = _.cloneDeep(this.selected)
        }
      }
      this.$toggle(select)
...
export default function transfer(opts){
  ...
  dft.props = {
    ...
    transferClass: dft.transferClass,
    header: dft.header,
    footer: dft.footer
  }
  return appTransfer(dft)
}
