...
// 初始化第三方库的实例
function init3DSLib(GVar, Instance, config, dom, opts, ctx){
  const upld_button = ctx.refs['upBtn']
  if(!Instance.loaded){
    if (Instance.uploader && Instance.uploader.destroy) Instance.uploader.destroy()
    else {
...
// 循环检测 WebUploader
let loopTimmer 
function loopDetection(Instance, config, dom, opts, ctx){
  clearTimeout(loopTimmer)
  loopTimmer = setTimeout(function() {
    if (typeof WebUploader !== 'undefined') {
      init3DSLib(WebUploader, Instance, config, dom, opts, ctx)
    } else {
      loopDetection(Instance, config, dom, opts, ctx)
    }
  }, 50);
}
  ...
  return function(options){
    ...
    try {
      inject.js('/js/t/webuploader', function(){
        loopDetection(MyUp, config, dom, opts, ctx)
      })
    } catch (error) {
